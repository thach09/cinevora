...
public class UserDataService {
    ...
    public UserDataService(UserRepository users, MovieRepository movies, WatchlistRepository watchlist, FavouriteRepository favourites,
                           ...
                           WatchProgressPolicy progressPolicy) {
        this.users = users; this.movies = movies; this.watchlist = watchlist; this.favourites = favourites; this.history = history; this.continueWatching = continueWatching; this.movieService = movieService; this.progressPolicy = progressPolicy;
    }
    ...
    @Transactional public void addHistory(String username, Long movieId) { User u = current(username); Movie m = activeMovie(movieId); if (!history.existsByUser_IdAndMovie_Id(u.getId(), movieId)) { history.save(new WatchHistory(u, m)); movieService.incrementViews(m); } }
    ...
    @Transactional public void updateProgress(String username, UserDataDtos.ProgressRequest request) {
        User u = current(username);
        Movie m = activeMovie(request.movieId());
        ...
        ContinueWatchingId id = new ContinueWatchingId(u.getId(), m.getId());
        if (progressPolicy.isComplete(percent, position, duration)) {
            continueWatching.deleteById(id);
            addHistory(username, movieId(m));
            return;
        }
        if (!progressPolicy.shouldContinue(percent, position, duration)) {
            continueWatching.deleteById(id);
            return;
        }
        ContinueWatching row = continueWatching.findById(id).orElseGet(() -> new ContinueWatching(u, m, percent, position, duration == 0 ? null : duration));
        row.setPercent(percent);
        row.setPositionSeconds(position);
        row.setDurationSeconds(duration == 0 ? null : duration);
        continueWatching.save(row);
    }
