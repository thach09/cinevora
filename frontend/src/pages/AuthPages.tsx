...
export function RegisterPage() {
  ...
  const submit = form.handleSubmit(async (values) => { setBusy(true); try { const auth = await authApi.register(values); queryClient.clear(); setAuth(auth); navigate('/browse', { replace: true }); push('Welcome to Cinevora!', 'success') } catch (error) { push(getApiError(error), 'error') } finally { setBusy(false) } })
